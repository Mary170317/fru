...
import { ShoppingCart, Plus, Minus, X, Phone, MapPin, Search, User, ExternalLink, Info, Check, AlertCircle, Upload, LogOut, Settings } from "lucide-react";
...
export default function Home() {
  ...
  const handleLogin = async () => {
    ...
    if (isRegistering) {
      ...
      if (!validatePhone(loginForm.phone)) return setLoginError("Некорректный телефон (+7XXXXXXXXXX)");
      ...
      try {
        ...
        if (loginForm.email === ADMIN_EMAIL) setIsAdmin(true);
        setShowLogin(false);
  ...
  const handleResetPassword = async () => {
    ...
    try { await resetPassword(resetEmail); setResetSent(true); setTimeout(() => { setResetSent(false); setForgotPassword(false); setResetEmail(""); }, 3000); }
    catch (e: any) { alert("Ошибка отправки письма."); }
  };
  ...
  // ============ АДМИНСКИЕ ФУНКЦИИ ============
  const saveToGitHub = async (updatedProducts: Product[], msg: string) => {
    setSaveStatus("⏳ Сохраняю...");
    try {
      const res = await fetch("/api/admin/save-products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ products: updatedProducts, message: msg }) });
      const result = await res.json();
      if (result.success) { setSaveStatus("✅ Сохранено! Сайт обновится через 1-2 минуты."); setProducts(updatedProducts); }
  ...
  const handleAddProduct = async () => {
    if (!newProduct.name || !newProduct.image || newProduct.price <= 0) { setSaveStatus("❌ Заполните название, цену и ссылку"); return; }
  ...
  const startEdit = (p: Product) => { setEditingId(p.id); setEditingProduct({ ...p }); };
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm("Удалить «" + (p?.name || id) + "»?")) return;
    const updated = products.filter(p => p.id !== id);
    await saveToGitHub(updated, "Удалён: " + (p?.name || id));
  };

  // ============ АДМИНСКИЙ РЕНДЕР ============
  if (isAdmin) {
    const adminFiltered = products.filter(p => (selectedCategory === "all" || p.category === selectedCategory) && p.name.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="min-h-screen bg-[#f5f3ef] font-sans">
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b shadow-sm">
          <div className="px-4 py-3 flex items-center justify-between max-w-5xl mx-auto">
            <div className="flex items-center gap-2"><span className="text-2xl">🍎</span><h1 className="text-lg font-bold text-[#4a7c59]">Админ-панель</h1><span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">{products.length} товаров</span></div>
            <div className="flex gap-2">
              <a href="/" target="_blank" className="text-sm text-blue-600 underline self-center">Сайт</a>
              <button onClick={handleLogout} className="bg-red-50 text-red-600 px-3 py-1.5 rounded-xl text-sm">Выйти</button>
            </div>
          </div>
        </header>
        <div className="px-4 py-4 max-w-5xl mx-auto">
          {saveStatus && <div className={"mb-3 p-3 rounded-xl text-sm " + (saveStatus.includes("✅") ? "bg-green-50 text-green-700" : saveStatus.includes("❌") ? "bg-red-50 text-red-700" : "bg-blue-50 text-blue-700")}>{saveStatus}</div>}
          <div className="bg-white rounded-2xl p-3 mb-4 flex flex-wrap gap-2 items-center">
            <input placeholder="Поиск..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 min-w-[120px] border rounded-xl px-3 py-2 text-sm" />
            <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="border rounded-xl px-3 py-2 text-sm">{<option value="all">Все</option>}{catList.map(c => <option key={c} value={c}>{catNames[c]}</option>)}</select>
            <button onClick={() => setIsAdding(true)} className="bg-[#4a7c59] text-white px-4 py-2 rounded-xl text-sm flex items-center gap-1"><Plus className="w-4 h-4" /> Добавить</button>
          </div>
          {isAdding && (
            <div className="bg-white rounded-2xl p-4 mb-4 border-2 border-green-200">
              <div className="flex justify-between mb-3"><h2 className="font-bold text-[#4a7c59]">Новый товар</h2><button onClick={() => setIsAdding(false)}><X className="w-5 h-5" /></button></div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <select value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})} className="border rounded-xl px-3 py-2 text-sm">{catList.map(c => <option key={c} value={c}>{catNames[c]}</option>)}</select>
                ...
                <input placeholder="Ед." value={newProduct.unit} onChange={e => setNewProduct({...newProduct, unit: e.target.value})} className="border rounded-xl px-3 py-2 text-sm" />
              ...
              <button onClick={handleAddProduct} className="mt-3 bg-[#4a7c59] text-white px-5 py-2 rounded-xl text-sm">Добавить</button>
            </div>
          )}
          <div className="bg-white rounded-2xl overflow-x-auto">
            <table className="w-full text-xs md:text-sm">
              <thead><tr className="bg-gray-50 border-b text-left"><th className="p-2">ID</th><th className="p-2">Фото</th><th className="p-2">Название</th><th className="p-2">Кат.</th><th className="p-2">Цена</th><th className="p-2"></th></tr></thead>
              <tbody>
                {adminFiltered.map(p => (
                  <tr key={p.id} className="border-b hover:bg-gray-50">
                    {editingId === p.id && editingProduct ? (
                      <>
                        ...
                        <td className="p-2"><input value={editingProduct.image} onChange={e => setEditingProduct({...editingProduct, image: e.target.value})} className="border rounded px-1 py-0.5 w-24 text-xs" /></td>
                        ...
                        <td className="p-2"><select value={editingProduct.category} onChange={e => setEditingProduct({...editingProduct, category: e.target.value})} className="border rounded px-1 py-0.5 text-xs">{catList.map(c => <option key={c} value={c}>{catNames[c]}</option>)}</select></td>
                        ...
                        <td className="p-2 flex gap-1"><button onClick={handleUpdate} className="p-1 bg-green-50 text-green-600 rounded"><Check className="w-3.5 h-3.5" /></button><button onClick={() => {setEditingId(null);setEditingProduct(null);}} className="p-1 bg-red-50 text-red-600 rounded"><X className="w-3.5 h-3.5" /></button></td>
                      </>
                    ) : (
                      <>
                        ...
                        <td className="p-2 text-xs">{catNames[p.category]}</td>
                        <td className="p-2 font-bold text-[#c0392b]">{p.price}₽</td>
                        <td className="p-2 flex gap-1"><button onClick={() => startEdit(p)} className="p-1 bg-blue-50 text-blue-600 rounded"><Edit className="w-3.5 h-3.5" /></button><button onClick={() => handleDelete(p.id)} className="p-1 bg-red-50 text-red-600 rounded"><Trash2 className="w-3.5 h-3.5" /></button></td>
                      </>
                    )}
                  </tr>
                ))}
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#faf8f5] to-[#f0ebe0] flex flex-col max-w-full overflow-x-hidden font-sans">
      <header className="sticky top-0 z-20 backdrop-blur-md bg-white/70 border-b border-orange-100 shadow-sm">
        <div className="px-3 md:px-4 py-2.5 md:py-4 flex items-center justify-between max-w-6xl mx-auto w-full">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="min-w-0"><h1 className="text-lg md:text-2xl font-bold text-[#4a7c59] truncate">Облачная 51</h1><p className="text-xs md:text-sm text-gray-500">Свежие продукты каждый день</p></div>
          </div>
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b">
        <div className="px-3 md:px-4 py-2 md:py-3 flex flex-wrap items-center gap-1.5 max-w-6xl mx-auto w-full">
          <span className="inline-flex items-center gap-1 text-xs md:text-sm text-gray-600 bg-orange-50/80 px-2 md:px-3 py-1.5 rounded-lg"><MapPin className="w-3 h-3 md:w-4 md:h-4 text-[#e87722] shrink-0" /> ул. Облачная, 51</span>
          <span className="inline-flex items-center gap-1 text-xs md:text-sm text-gray-600 bg-orange-50/80 px-2 md:px-3 py-1.5 rounded-lg"><Phone className="w-3 h-3 md:w-4 md:h-4 text-[#e87722] shrink-0" /> +7 913 004 1112</span>
          <button onClick={() => setIsMapVisible(true)} className="text-xs md:text-sm bg-green-50/80 text-[#4a7c59] px-2 md:px-4 py-1.5 rounded-lg font-medium hover:bg-green-100/80 inline-flex items-center gap-1"><MapPin className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Наш магазин</button>
          <a href="https://t.me/fruktiovoshiOblachnaya51" target="_blank" className="text-xs md:text-sm bg-blue-50/80 text-[#1565c0] px-2 md:px-4 py-1.5 rounded-lg font-medium hover:bg-blue-100/80 inline-flex items-center gap-1"><ExternalLink className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Telegram</a>
        </div>
      </div>
      {isLoggedIn && (
        <div className="bg-yellow-50/80 backdrop-blur-sm border-b border-orange-100">
          <div className="px-3 md:px-4 py-2 flex gap-2 items-center max-w-6xl mx-auto w-full flex-wrap">
            ...
            <input type="text" placeholder="📍 Адрес доставки" value={userAddress} onChange={e => { setUserAddress(e.target.value); setAddressConfirmed(false); }} className="bg-white/90 border border-orange-200 rounded-lg px-3 py-2 outline-none flex-1 text-xs md:text-sm min-w-[120px]" />
            <button onClick={confirmAddress} className={"font-medium text-xs px-3 py-2 rounded-lg flex items-center gap-1 shrink-0 " + (addressConfirmed ? "bg-green-500 text-white" : "bg-[#e87722] text-white")}>{addressConfirmed ? "✅ Подтверждён" : "Подтвердить"}</button>
          </div>
      ...
      <div className="px-3 md:px-4 py-3 md:py-6 flex gap-3 md:gap-6 flex-1 max-w-7xl mx-auto w-full">
        <div className="hidden md:flex flex-col gap-1.5 w-48 md:w-56 shrink-0">
          ...
          {categories.map(c => (<button key={c.id} onClick={() => setSelectedCategory(c.id)} className={"text-left px-3 md:px-4 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-medium text-xs md:text-sm transition-all " + (selectedCategory === c.id ? "bg-[#e87722] text-white shadow-lg" : "bg-white/70 text-gray-700 hover:bg-orange-50/80")}>{c.name}</button>))}
          <div className="mt-4 bg-white/70 rounded-2xl p-3 md:p-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="md:hidden flex gap-1.5 overflow-x-auto pb-2 mb-3 -mx-1 px-1 scrollbar-hide">{categories.map(c => (<button key={c.id} onClick={() => setSelectedCategory(c.id)} className={"whitespace-nowrap px-3 py-1.5 rounded-lg text-xs font-medium transition-all shrink-0 " + (selectedCategory === c.id ? "bg-[#e87722] text-white shadow" : "bg-white/70 text-gray-600")}>{c.name}</button>))}</div>
          <div className="flex items-center gap-2 bg-white/70 border border-white/50 rounded-xl px-3 py-2.5 mb-3"><Search className="w-4 h-4 text-gray-400 shrink-0" /><input type="text" placeholder="🔍 Поиск товаров..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-transparent outline-none flex-1 text-sm min-w-0" /></div>
          {filtered.length === 0 ? <div className="text-center py-12 text-gray-400 bg-white/70 rounded-2xl"><span className="text-3xl block mb-3">🍃</span><p className="text-sm">Ничего не найдено</p></div> : (
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-4">
              {filtered.map(p => (
                <div key={p.id} className="bg-white/70 rounded-xl md:rounded-3xl shadow-sm border overflow-hidden hover:shadow-md transition-all group">
                  <div className="aspect-square bg-gradient-to-br from-green-50/50 to-orange-50/50 relative overflow-hidden"><img src={getProductImage(p)} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy" /></div>
                  <div className="p-2 md:p-4">
                    ...
                    <button onClick={() => setSelectedProduct(p)} className="mt-2 md:mt-3 w-full text-xs text-[#4a7c59] font-medium flex items-center justify-center gap-1 py-1.5 md:py-2 rounded-lg md:rounded-xl bg-green-50/80 hover:bg-green-100/80 transition-all"><Info className="w-3 h-3 md:w-3.5 md:h-3.5 shrink-0" /> Характеристики</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="bg-white/80 border-t mt-auto"><div className="px-4 py-6 md:py-10 max-w-6xl mx-auto w-full"><div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8"><div><h3 className="text-lg md:text-xl font-bold mb-2 flex items-center gap-2"><span className="text-xl md:text-2xl">🍎</span> Облачная 51</h3><p className="text-gray-500 text-xs md:text-sm">Свежие продукты с доставкой по Новосибирску.</p></div><div><h4 className="font-semibold mb-2 md:mb-3 text-[#4a7c59] text-sm">Контакты</h4><ul className="space-y-1.5 text-xs md:text-sm text-gray-500"><li className="flex items-center gap-2"><MapPin className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> ул. Облачная, 51</li><li className="flex items-center gap-2"><Phone className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> +7 913 004 1112</li><li>🕒 Ежедневно с 9:00 до 21:00</li></ul></div><div><h4 className="font-semibold mb-2 md:mb-3 text-[#4a7c59] text-sm">Telegram</h4><a href="https://t.me/fruktiovoshiOblachnaya51" target="_blank" className="inline-flex items-center gap-2 bg-blue-50/80 text-[#1565c0] px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs md:text-sm font-medium hover:bg-blue-100/80 transition-all"><ExternalLink className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Подписаться</a></div></div><div className="border-t border-gray-200 mt-6 md:mt-8 pt-4 md:pt-6 text-center text-xs md:text-sm text-gray-400"><p>© 2026 Облачная 51. Все права защищены.</p></div></div></footer>
      ...
      {isMapVisible && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setIsMapVisible(false)}>
          <div className="bg-white rounded-2xl md:rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 md:p-5"><h2 className="font-bold text-base md:text-lg text-[#4a7c59]">📍 ул. Облачная, 51</h2><button onClick={() => setIsMapVisible(false)} className="p-1"><X className="w-5 h-5" /></button></div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white rounded-2xl md:rounded-3xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 md:p-5"><img src={getProductImage(selectedProduct)} alt={selectedProduct.name} className="w-full h-44 md:h-48 object-cover rounded-xl md:rounded-2xl mb-4" /><p className="text-gray-700 text-sm mb-3">{selectedProduct.description || "Описание скоро появится."}</p>{selectedProduct.benefits && <div className="bg-green-50 rounded-xl md:rounded-2xl p-3 md:p-4 mt-3"><p className="text-sm font-semibold text-[#4a7c59] mb-1">💚 Польза:</p><p className="text-sm text-gray-700">{selectedProduct.benefits}</p></div>}</div>
          </div>
        </div>
      )}
            ...
            {forgotPassword ? (
              <>
                ...
                {resetSent ? <div className="text-center py-8"><span className="text-4xl md:text-5xl block mb-3">📧</span><p className="text-green-600 font-medium">Письмо отправлено!</p></div> : (
                  <>
                    <p className="text-sm text-gray-500 mb-4 text-center">Введите email для сброса пароля.</p>
              ...
              <>
                ...
                <input type="email" placeholder="Email" value={loginForm.email} onChange={e => setLoginForm({...loginForm, email: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 mb-3 outline-none focus:border-[#4a7c59] text-sm" />
      ...
      {showPayment && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setShowPayment(false)}>
          <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-green-50 rounded-xl p-4 mb-4"><p className="text-sm font-semibold text-[#4a7c59] mb-2">Реквизиты:</p>{banks.map((bank, idx) => (<p key={idx} className="text-sm"><strong>{bank.name}:</strong> {bank.phone}</p>))}</div>
            <label className="flex items-center gap-2 w-full border-2 border-dashed border-gray-300 rounded-xl px-4 py-4 mb-4 text-sm text-gray-500 cursor-pointer hover:border-[#4a7c59] transition-all text-center justify-center"><Upload className="w-4 h-4" />{paymentScreenshot ? paymentScreenshot.name : "Прикрепить чек"}<input type="file" accept="image/*" onChange={(e) => setPaymentScreenshot(e.target.files?.[0] || null)} className="hidden" /></label>
            <button onClick={handleOrder} disabled={!paymentScreenshot} className="w-full bg-[#e87722] text-white py-3.5 rounded-xl font-medium hover:bg-orange-600 transition-all disabled:opacity-50">✅ Подтвердить оплату</button>
          </div>
        </div>
      )}
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5">{cart.length === 0 ? <p className="text-gray-500 text-center py-12 md:py-16">Корзина пуста</p> : cart.map(i => (
              <div key={i.id} className="flex items-center gap-3 border-b pb-3">
                ...
                <div className="flex-1 min-w-0"><h4 className="font-medium text-sm truncate">{i.name}</h4><p className="text-[#c0392b] font-bold text-sm">{i.price} ₽ / {i.unit}</p><p className="text-xs text-gray-500">{i.quantity} × {i.price} ₽ = {i.quantity * i.price} ₽</p></div>
