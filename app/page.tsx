      ...
      {isCartOpen && (
        <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setIsCartOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5">{cart.length === 0 ? <p className="text-gray-500 text-center py-12 md:py-16">Корзина пуста</p> : cart.map(i => (<div key={i.id} className="flex items-center gap-3 border-b pb-3"><img src={getProductImage(i)} alt={i.name} className="w-14 h-14 md:w-16 md:h-16 object-cover rounded-lg md:rounded-xl shrink-0" /><div className="flex-1 min-w-0"><h4 className="font-medium text-sm truncate">{i.name}</h4><p className="text-[#c0392b] font-bold text-sm">{i.price} ₽ / {i.unit}</p></div><div className="flex items-center gap-2 shrink-0"><button onClick={() => updQty(i.id, -1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200"><Minus className="w-3.5 h-3.5 md:w-4 md:h-4" /></button><span className="w-5 md:w-6 text-center font-medium text-sm">{i.quantity}</span><button onClick={() => updQty(i.id, 1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[#e87722] text-white flex items-center justify-center hover:bg-orange-600"><Plus className="w-3.5 h-3.5 md:w-4 md:h-4" /></button></div></div>))}</div>
