...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0] flex flex-col max-w-full overflow-x-hidden">
      ...
      <div className="px-3 md:px-4 py-3 md:py-6 flex gap-3 md:gap-6 flex-1 max-w-6xl mx-auto w-full">
        <div className="hidden md:flex flex-col gap-1.5 w-48 md:w-56 shrink-0">
          ...
          <div className="mt-4 bg-white rounded-2xl p-3 md:p-4 border">
            ...
            {deliveryZones.map(z => (<button key={z.id} onClick={() => setSelectedZone(z.id)} className={`w-full text-left px-2 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs font-medium mb-1 transition-all ${selectedZone === z.id ? "bg-[#e87722] text-white" : "bg-gray-50 text-gray-600 hover:bg-orange-50"}`}>{z.name}: {z.price === 0 ? (z.note || "Бесплатно") : `${z.price} ₽`}</button>))}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="md:hidden mb-3 bg-white rounded-xl p-2 border">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {deliveryZones.map(z => (<button key={z.id} onClick={() => setSelectedZone(z.id)} className={`px-2 py-1 rounded-lg text-xs transition-all shrink-0 ${selectedZone === z.id ? "bg-[#e87722] text-white" : "bg-gray-50 text-gray-600"}`}>{z.name}: {z.price === 0 ? (z.note || "0₽") : `${z.price}₽`}</button>))}
            </div>
          </div>
                ...
                <div key={p.id} className="bg-white rounded-xl md:rounded-3xl shadow-sm border overflow-hidden hover:shadow-md transition-all group">
                  ...
                  <div className="p-2 md:p-4">
                    ...
                    <div className="flex items-center justify-between mt-2 md:mt-3">
                      ...
                      <button onClick={() => addToCart(p)} className="bg-[#e87722] text-white rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shadow-md hover:bg-orange-600 transition-all shrink-0"><Plus className="w-4 h-4 md:w-5 md:h-5" /></button>
                    </div>
                    <button onClick={() => setSelectedProduct(p)} className="mt-2 md:mt-3 w-full text-xs text-[#4a7c59] font-medium flex items-center justify-center gap-1 py-1.5 md:py-2 rounded-lg md:rounded-xl bg-green-50 hover:bg-green-100 transition-all"><Info className="w-3 h-3 md:w-3.5 md:h-3.5 shrink-0" /> Характеристики</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="bg-[#2c3e50] text-white mt-auto">
        <div className="px-4 py-6 md:py-10 max-w-6xl mx-auto w-full">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            <div><h3 className="text-lg md:text-xl font-bold mb-2 flex items-center gap-2"><span className="text-xl md:text-2xl">🍎</span> Облачная 51</h3><p className="text-gray-300 text-xs md:text-sm">Свежие продукты с доставкой по Новосибирску.</p></div>
            <div><h4 className="font-semibold mb-2 md:mb-3 text-green-300 text-sm">Контакты</h4><ul className="space-y-1.5 text-xs md:text-sm text-gray-300"><li className="flex items-center gap-2"><MapPin className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> ул. Облачная, 51</li><li className="flex items-center gap-2"><Phone className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> +7 913 004 1112</li></ul></div>
            <div><h4 className="font-semibold mb-2 md:mb-3 text-green-300 text-sm">Telegram</h4><a href="https://t.me/fruktiovoshiOblachnaya51" target="_blank" className="inline-flex items-center gap-2 bg-[#0088cc] text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs md:text-sm font-medium hover:bg-blue-600 transition-all"><ExternalLink className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Подписаться</a></div>
          </div>
          <div className="border-t border-gray-600 mt-6 md:mt-8 pt-4 md:pt-6 text-center text-xs md:text-sm text-gray-400"><p>© 2026 Фрукты & Овощи | Облачная 51</p></div>
        </div>
      </footer>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white rounded-2xl md:rounded-3xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 md:p-5"><img src={getProductImage(selectedProduct)} alt={selectedProduct.name} className="w-full h-44 md:h-48 object-cover rounded-xl md:rounded-2xl mb-4" /><p className="text-gray-700 text-sm mb-3">{selectedProduct.description || "Описание скоро появится."}</p>{selectedProduct.benefits && <div className="bg-green-50 rounded-xl md:rounded-2xl p-3 md:p-4"><p className="text-sm font-semibold text-[#4a7c59] mb-1">💚 Польза:</p><p className="text-sm text-gray-700">{selectedProduct.benefits}</p></div>}<div className="mt-4 flex items-center justify-between"><span className="text-xl md:text-2xl font-bold text-[#c0392b]">{selectedProduct.price > 0 ? `${selectedProduct.price} ₽` : "—"}</span><button onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }} className="bg-[#e87722] text-white px-5 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-medium hover:bg-orange-600 transition-all">🛒 В корзину</button></div></div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            {forgotPassword ? (
              ...
              <>
                ...
                {isRegistering && <input type="tel" placeholder="Телефон" value={loginForm.phone} onChange={e => setLoginForm({...loginForm, phone: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 mb-3 outline-none focus:border-[#4a7c59] text-sm" />}
      ...
      {/* МОДАЛКА ОПЛАТЫ С ВОЗМОЖНОСТЬЮ ПРИКРЕПИТЬ ЧЕК */}
      {showPayment && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setShowPayment(false)}>
          <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-gray-500 mb-3">
              После оплаты, прикрепите скриншот чека и нажмите «Оформить заказ».
            </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setIsCartOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5">
              {cart.length === 0 ? <p className="text-gray-500 text-center py-12 md:py-16">Корзина пуста</p> : cart.map(i => (
                <div key={i.id} className="flex items-center gap-3 border-b pb-3"><img src={getProductImage(i)} alt={i.name} className="w-14 h-14 md:w-16 md:h-16 object-cover rounded-lg md:rounded-xl shrink-0" /><div className="flex-1 min-w-0"><h4 className="font-medium text-sm truncate">{i.name}</h4><p className="text-[#c0392b] font-bold text-sm">{i.price} ₽ / {i.unit}</p></div><div className="flex items-center gap-2 shrink-0"><button onClick={() => updQty(i.id, -1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200"><Minus className="w-3.5 h-3.5 md:w-4 md:h-4" /></button><span className="w-5 md:w-6 text-center font-medium text-sm">{i.quantity}</span><button onClick={() => updQty(i.id, 1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[#e87722] text-white flex items-center justify-center hover:bg-orange-600"><Plus className="w-3.5 h-3.5 md:w-4 md:h-4" /></button></div></div>
              ))}
            </div>
