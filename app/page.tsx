    ...
    <div className="min-h-screen bg-[#FFF8F0] flex flex-col max-w-full overflow-x-hidden">
      ...
      <header className="sticky top-0 z-20 bg-[#FFF8F0] border-b border-orange-100 shadow-sm">
        <div className="px-3 md:px-4 py-2.5 md:py-4 flex items-center justify-between max-w-6xl mx-auto w-full">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="bg-orange-100 p-1.5 md:p-2 rounded-xl md:rounded-2xl shrink-0">
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0">
            {isLoggedIn && (
              <span className="text-xs md:text-sm bg-orange-50 px-2 md:px-3 py-1 md:py-1.5 rounded-full hidden sm:block truncate max-w-[100px]">
                👋 {userName || userEmail}
              </span>
            )}
            <button onClick={() => isLoggedIn ? handleLogout() : setShowLogin(true)} className="p-2 md:p-2.5 hover:bg-orange-50 rounded-full">
              <User className="w-4 h-4 md:w-5 md:h-5" />
            </button>
            <button onClick={() => setIsCartOpen(true)} className="relative p-2 md:p-2.5 hover:bg-orange-50 rounded-full">
              <ShoppingCart className="w-4 h-4 md:w-5 md:h-5" />
              {items > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-[#e87722] text-white text-xs w-4 h-4 md:w-5 md:h-5 rounded-full flex items-center justify-center font-bold">
                  {items}
                </span>
              )}
            </button>
      ...
      <div className="bg-white border-b">
        <div className="px-3 md:px-4 py-2 md:py-3 flex flex-wrap items-center gap-1.5 max-w-6xl mx-auto w-full">
          <span className="inline-flex items-center gap-1 text-xs md:text-sm text-gray-600 bg-orange-50 px-2 md:px-3 py-1.5 rounded-lg">
            <MapPin className="w-3 h-3 md:w-4 md:h-4 text-[#e87722] shrink-0" />
            <span className="truncate max-w-[120px] md:max-w-none">ул. Облачная, 51</span>
          </span>
          <span className="inline-flex items-center gap-1 text-xs md:text-sm text-gray-600 bg-orange-50 px-2 md:px-3 py-1.5 rounded-lg">
            <Phone className="w-3 h-3 md:w-4 md:h-4 text-[#e87722] shrink-0" />
            +7 913 004 1112
          </span>
          <button onClick={() => setIsMapVisible(true)} className="text-xs md:text-sm bg-[#e8f5e9] text-[#4a7c59] px-2 md:px-4 py-1.5 rounded-lg font-medium hover:bg-[#c8e6c9] inline-flex items-center gap-1">
            <MapPin className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Наш магазин
          </button>
          <a href="https://t.me/fruktiovoshiOblachnaya51" target="_blank" className="text-xs md:text-sm bg-[#e3f2fd] text-[#1565c0] px-2 md:px-4 py-1.5 rounded-lg font-medium hover:bg-[#bbdefb] inline-flex items-center gap-1">
            <ExternalLink className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Telegram
          </a>
        </div>
      </div>
        ...
        <div className="bg-[#FFF8E1] border-b border-orange-100">
          <div className="px-3 md:px-4 py-2 flex gap-2 items-center max-w-6xl mx-auto w-full flex-wrap">
            ...
            <input
              ...
              value={userAddress}
              onChange={e => { setUserAddress(e.target.value); setAddressConfirmed(false); }}
              className="bg-white border border-orange-200 rounded-lg px-3 py-2 outline-none flex-1 text-xs md:text-sm min-w-[120px]"
            />
            <button
              onClick={confirmAddress}
              className={`font-medium text-xs px-3 py-2 rounded-lg flex items-center gap-1 shrink-0 ${addressConfirmed ? "bg-green-500 text-white" : "bg-[#e87722] text-white"}`}
            >
              {addressConfirmed ? <><Check className="w-3 h-3" /> ✓</> : "Подтвердить"}
            </button>
          </div>
          {addressError && (
            <div className="px-3 pb-2 text-red-500 text-xs flex items-center gap-1">
              <AlertCircle className="w-3 h-3 shrink-0" /> {addressError}
            </div>
          )}
        </div>
      )}
        ...
        <div className="hidden md:flex flex-col gap-1.5 w-48 md:w-56 shrink-0">
          ...
          {categories.map(c => (
            <button
              ...
              className={`text-left px-3 md:px-4 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-medium text-xs md:text-sm transition-all ${selectedCategory === c.id ? "bg-[#e87722] text-white shadow-lg" : "bg-white text-gray-700 hover:bg-orange-50 border"}`}
            >
              {c.name}
            </button>
          ))}
          <div className="mt-4 bg-white rounded-2xl p-3 md:p-4 border">
            <h3 className="font-bold text-[#4a7c59] text-xs md:text-sm mb-2">🚚 Доставка</h3>
            {deliveryZones.map(z => (
              <button
                key={z.id}
                onClick={() => setSelectedZone(z.id)}
                className={`w-full text-left px-2 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs font-medium mb-1 transition-all ${selectedZone === z.id ? "bg-[#e87722] text-white" : "bg-gray-50 text-gray-600 hover:bg-orange-50"}`}
              >
                {z.name}: {z.price === 0 ? (z.note || "Бесплатно") : `${z.price} ₽`}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="md:hidden flex gap-1.5 overflow-x-auto pb-2 mb-3 -mx-1 px-1 scrollbar-hide">
            {categories.map(c => (
              <button
                key={c.id}
                onClick={() => setSelectedCategory(c.id)}
                className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-xs font-medium transition-all shrink-0 ${selectedCategory === c.id ? "bg-[#e87722] text-white shadow" : "bg-white text-gray-600 border"}`}
              >
                {c.name}
              </button>
            ))}
            ...
            <div className="flex gap-1.5 flex-wrap">
              {deliveryZones.map(z => (
                <button
                  key={z.id}
                  onClick={() => setSelectedZone(z.id)}
                  className={`px-2 py-1 rounded-lg text-xs transition-all shrink-0 ${selectedZone === z.id ? "bg-[#e87722] text-white" : "bg-gray-50 text-gray-600"}`}
                >
                  {z.name}: {z.price === 0 ? (z.note || "0₽") : `${z.price}₽`}
                </button>
              ))}
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4">
              {filtered.map(p => (
                <div key={p.id} className="bg-white rounded-xl md:rounded-3xl shadow-sm border overflow-hidden hover:shadow-md transition-all group">
                  <div className="aspect-square bg-gradient-to-br from-green-50 to-orange-50 relative overflow-hidden">
                    <img
                  ...
                  <div className="p-2 md:p-4">
                    ...
                    <div className="flex items-center justify-between mt-2 md:mt-3">
                      ...
                      <button
                        onClick={() => addToCart(p)}
                        className="bg-[#e87722] text-white rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shadow-md hover:bg-orange-600 transition-all shrink-0"
                      >
                    ...
                    <button
                      ...
                      className="mt-2 md:mt-3 w-full text-xs text-[#4a7c59] font-medium flex items-center justify-center gap-1 py-1.5 md:py-2 rounded-lg md:rounded-xl bg-green-50 hover:bg-green-100 transition-all"
                    >
                      <Info className="w-3 h-3 md:w-3.5 md:h-3.5 shrink-0" /> Характеристики
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            <div>
              <h3 className="text-lg md:text-xl font-bold mb-2 flex items-center gap-2">
                <span className="text-xl md:text-2xl">🍎</span> Облачная 51
              </h3>
              <p className="text-gray-300 text-xs md:text-sm">Свежие продукты с доставкой по Новосибирску.</p>
            </div>
            <div>
              <h4 className="font-semibold mb-2 md:mb-3 text-green-300 text-sm">Контакты</h4>
              <ul className="space-y-1.5 text-xs md:text-sm text-gray-300">
                <li className="flex items-center gap-2"><MapPin className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> ул. Облачная, 51</li>
                <li className="flex items-center gap-2"><Phone className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> +7 913 004 1112</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-2 md:mb-3 text-green-300 text-sm">Telegram</h4>
              <a
                href="https://t.me/fruktiovoshiOblachnaya51"
                target="_blank"
                className="inline-flex items-center gap-2 bg-[#0088cc] text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs md:text-sm font-medium hover:bg-blue-600 transition-all"
              >
                <ExternalLink className="w-3 h-3 md:w-4 md:h-4 shrink-0" /> Подписаться
              </a>
            </div>
          </div>
          <div className="border-t border-gray-600 mt-6 md:mt-8 pt-4 md:pt-6 text-center text-xs md:text-sm text-gray-400">
            <p>© 2026 Фрукты & Овощи | Облачная 51</p>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setIsMapVisible(false)}>
          <div className="bg-white rounded-2xl md:rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 md:p-5">
              <h2 className="font-bold text-base md:text-lg text-[#4a7c59]">📍 ул. Облачная, 51 — магазин</h2>
              <button onClick={() => setIsMapVisible(false)} className="p-1"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-4 md:p-5 text-sm text-gray-600">
              <p>📍 <strong>ул. Облачная, 51</strong>, Новосибирск</p>
              <div className="mt-2 space-y-1">
                <p className="font-medium text-[#4a7c59]">Зоны доставки:</p>
                {deliveryZones.map(z => (
                  <div key={z.id} className="flex items-center gap-2 text-xs md:text-sm">
                    <div className="w-3 h-3 rounded-full shrink-0" style={{backgroundColor: z.color}}></div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-3" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white rounded-2xl md:rounded-3xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 md:p-5 border-b">
              <h2 className="font-bold text-base md:text-lg text-[#4a7c59] truncate">{selectedProduct.name}</h2>
              <button onClick={() => setSelectedProduct(null)} className="p-1 shrink-0"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-4 md:p-5">
              <img
                src={getProductImage(selectedProduct)}
                alt={selectedProduct.name}
                className="w-full h-44 md:h-48 object-cover rounded-xl md:rounded-2xl mb-4"
              />
              ...
              {selectedProduct.benefits && (
                <div className="bg-green-50 rounded-xl md:rounded-2xl p-3 md:p-4">
                  <p className="text-sm font-semibold text-[#4a7c59] mb-1">💚 Польза:</p>
                  <p className="text-sm text-gray-700">{selectedProduct.benefits}</p>
                </div>
              )}
              <div className="mt-4 flex items-center justify-between">
                <span className="text-xl md:text-2xl font-bold text-[#c0392b]">
                  {selectedProduct.price > 0 ? `${selectedProduct.price} ₽` : "—"}
                </span>
                <button
                  onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }}
                  className="bg-[#e87722] text-white px-5 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-medium hover:bg-orange-600 transition-all"
                >
                  🛒 В корзину
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-3" onClick={() => { setShowLogin(false); setForgotPassword(false); }}>
          <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            {forgotPassword ? (
              <>
                ...
                {resetSent ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-green-600 font-medium">Письмо отправлено!</p>
                  </div>
                ) : (
                  <>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-200 rounded-xl px-4 py-3 mb-4 outline-none focus:border-[#4a7c59] text-sm"
                    />
                    <button onClick={handleResetPassword} className="w-full bg-[#4a7c59] text-white py-3 rounded-xl font-medium hover:bg-green-700 transition-all">
                      Отправить ссылку
                    </button>
                  </>
                )}
                <p className="text-center text-sm text-gray-500 mt-4">
                  <button onClick={() => { setForgotPassword(false); setResetSent(false); }} className="text-[#4a7c59] font-medium underline">
                    ← Вернуться ко входу
                  </button>
                </p>
              </>
              ...
              <>
                <h2 className="text-lg md:text-xl font-bold mb-4 text-center text-[#4a7c59]">
                ...
                {isRegistering && (
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 mb-3 outline-none focus:border-[#4a7c59] text-sm"
                  />
                )}
                <input
                  type="email"
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 mb-4 outline-none focus:border-[#4a7c59] text-sm"
                />
                <button onClick={handleLogin} className="w-full bg-[#e87722] hover:bg-orange-600 text-white py-3.5 rounded-xl font-medium transition-all shadow-lg shadow-orange-200">
                  {isRegistering ? "Зарегистрироваться" : "Войти"}
                </button>
                {!isRegistering && (
                  <p className="text-center text-sm text-gray-500 mt-3">
                    <button onClick={() => setForgotPassword(true)} className="text-[#4a7c59] font-medium underline">Забыли пароль?</button>
                  </p>
                )}
                <p className="text-center text-sm text-gray-500 mt-3">
                  {isRegistering ? "Уже есть аккаунт?" : "Нет аккаунта?"}{" "}
                  <button onClick={() => setIsRegistering(!isRegistering)} className="text-[#4a7c59] font-medium underline">
      ...
      {isCartOpen && (
        <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setIsCartOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 md:p-5 bg-[#4a7c59] text-white">
              <h2 className="text-lg md:text-xl font-bold flex items-center gap-2">
                <ShoppingCart className="w-5 h-5" /> Корзина
              </h2>
              <button onClick={() => setIsCartOpen(false)} className="p-1 hover:bg-white/20 rounded-full"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 md:p-5">
              {cart.length === 0 ? (
                <p className="text-gray-500 text-center py-12 md:py-16">Корзина пуста</p>
              ) : (
                cart.map(i => (
                  <div key={i.id} className="flex items-center gap-3 border-b pb-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button onClick={() => updQty(i.id, -1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200">
                        <Minus className="w-3.5 h-3.5 md:w-4 md:h-4" />
                      </button>
                      <span className="w-5 md:w-6 text-center font-medium text-sm">{i.quantity}</span>
                      <button onClick={() => updQty(i.id, 1)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[#e87722] text-white flex items-center justify-center hover:bg-orange-600">
            ...
            <div className="border-t p-4 md:p-5 bg-green-50">
              <div className="flex items-center justify-between mb-4">
                <span className="text-base md:text-lg font-medium">Итого:</span>
                <span className="text-xl md:text-2xl font-bold text-[#c0392b]">{total} ₽</span>
              </div>
              <button
                onClick={handleOrder}
